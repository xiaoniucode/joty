...
const onCreate = async () => {
  ...
  api.action(short_url.restoreUrl, {}, { shortUrl: url.value.trim() }).then((res: any) => {
    result.value = res
  ...
  <a-flex style="padding: 10% 5%" vertical gap="50">
    <a-flex gap="8" align="center">
      <a-input
        ...
        size="large"
        v-model:value="url"
        placeholder="输入一个http或https开头的短链接还原出原始链接"
      />
      <a-button @click="onCreate" type="primary" size="large">立即还原</a-button>
    </a-flex>
